...
using CmsApp.Data;
using CmsApp.HealthChecks;
using CmsApp.Localization;
using CmsApp.Menus;
using CmsApp.Pages.Shared.Components.Footer;
...
public class CmsAppModule : AbpModule
{
    ...
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        Configure<AbpLayoutHookOptions>(options =>
        {
            options.Add(
                ...
                typeof(CmsAppFooterViewComponent)
            );
        });
    ...
    private void ConfigureLocalization()
    {
        ...
        {
            options.Resources
                .Add<CmsAppResource>("en")
                .AddBaseTypes(typeof(AbpValidationResource))
                .AddVirtualJson("/Localization/CmsApp");

            options.DefaultResourceType = typeof(CmsAppResource);
        ...
        {
            options.MapCodeNamespace("CmsApp", typeof(CmsAppResource));
        });
    }
    ...
    private void ConfigureVirtualFiles(IWebHostEnvironment hostingEnvironment)
    {
        ...
        {
            options.FileSets.AddEmbedded<CmsAppModule>();
            if (hostingEnvironment.IsDevelopment())
            {
                /* Using physical files in development, so we don't need to recompile on changes */
                options.FileSets.ReplaceEmbeddedByPhysical<CmsAppModule>(hostingEnvironment.ContentRootPath);
            }
        });
    }
    ...
    private void ConfigureNavigationServices()
    {
        ...
        {
            options.MenuContributors.Add(new CmsAppMenuContributor());
        });
    }
    ...
    private void ConfigureAutoApiControllers()
    {
        ...
        {
            options.ConventionalControllers.Create(typeof(CmsAppModule).Assembly);
        });
    }
    ...
    private void ConfigureSwagger(IServiceCollection services)
    {
        services.AddAbpSwaggerGen(
            options =>
            {
                options.SwaggerDoc("v1", new OpenApiInfo { Title = "CmsApp API", Version = "v1" });
    ...
    {
        context.Services.AddAutoMapperObjectMapper<CmsAppModule>();
        Configure<AbpAutoMapperOptions>(options =>
        {
             ...
             */
            options.AddMaps<CmsAppModule>(/* validate: true */);
        });
    }

    private void ConfigureEfCore(ServiceConfigurationContext context)
    {
        context.Services.AddAbpDbContext<CmsAppDbContext>(options =>
        {
        ...
        Configure<AbpDbContextOptions>(options =>
        {
            options.Configure(configurationContext =>
            {
                configurationContext.UseSqlite()
                    .ReplaceService<IMethodCallTranslatorProvider, CmsAppSqliteMethodCallTranslatorProvider>();
            });
        });
    ...
    {
        DirectoryHelper.CreateIfNotExists(context.GetConfiguration()["App:DbFolderName"] ?? "CmsAppDb");
        var connString = await context.ServiceProvider.GetRequiredService<IConnectionStringResolver>().ResolveAsync();
        await context.ServiceProvider.GetRequiredService<CmsAppDbMigrationService>().MigrateAsync(connString);
    }

    private void ConfigureHealthChecks(ServiceConfigurationContext context)
    {
        context.Services.AddCmsAppHealthChecks();
    }
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        {
            options.SwaggerEndpoint("/swagger/v1/swagger.json", "CmsApp API");
        });
